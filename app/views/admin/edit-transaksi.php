...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Get the transaction ID and new status from the form
    $NoTransaksi = $_POST['NoTransaksi'];
    $StatusTransaksi = $_POST['StatusTransaksi'];

    // Update the status in the database
    $sql = "UPDATE viewriwayattransaksi SET StatusTransaksi = '$StatusTransaksi' WHERE NoTransaksi = '$NoTransaksi'";
    $query = mysqli_query($db, $sql);

    // Redirect to the transaction list page after updating
...
// Get the transaction details from the database based on the provided ID
$NoTransaksi = $_GET['NoTransaksi'];
$sql = "SELECT * FROM viewriwayattransaksi WHERE NoTransaksi = '$NoTransaksi'";
$query = mysqli_query($db, $sql);
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                        <input type="hidden" name="NoTransaksi" value="<?php echo $transaksi['NoTransaksi']; ?>">
                        <div class="form-group">
                            <label for="statusTransaksi" style="font-size: 16px; font-weight: bold;">Status Transaksi:</label>
                            <select class="form-control" name="StatusTransaksi" id="statusTransaksi" style="font-size: 14px;">
                                <option value="Proses" <?php if ($transaksi['StatusTransaksi'] == 'Proses') echo 'selected'; ?>>Proses</option>
                                <option value="Mulai" <?php if ($transaksi['StatusTransaksi'] == 'Mulai') echo 'selected'; ?>>Mulai</option>
                                <option value="Batal" <?php if ($transaksi['StatusTransaksi'] == 'Batal') echo 'selected'; ?>>Batal</option>
                                <option value="Arsip" <?php if ($transaksi['StatusTransaksi'] == 'Arsip') echo 'selected'; ?>>Arsip</option>
                                <option value="Selesai" <?php if ($transaksi['StatusTransaksi'] == 'Selesai') echo 'selected'; ?>>Selesai</option>
                            </select>
                        </div>
                        <button type="submit" class="btn btn-primary mt-2">Update</button>
                        <a href="data-transaksi.php" class="btn btn-secondary mt-2">Cancel</a>
                    </form>
                </div>
                <!-- /.card-body -->
            </div>
